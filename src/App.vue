...
/* flow 
 watch the value of the let openmeteogeo.name  when it changes uses updatelist() 
 when user choose from the search output it will change the value of listvar.value and be 
 detected by another (watch) it will then use forfinalresult()  the important part there is the lat and long so the place 
 (button click) it will then run mainweatherfetch() and get the weather data.
*/
...
// got this from the github above for changing the description and image based on weather condition (wmo codes)
const wmo = reactive({
  0: {
    day: {
      description: "Sunny",
...
// use for link of openstreet when it will be used  below it will have the lat and long of the chosen location
let link = reactive({
  //https://www.openstreetmap.org/#map=13/15.15000/120.58333
  linkname: "https://www.openstreetmap.org/#map=13/",
  add: "/",
});
// set a link that i can use for the template below  the this will be changed when choosing from the search outputs
...
// fetch value from the api, the openmeteogeo.name is differenct depending on what the user choose
function updatelist() {
...
/*
 this is use so the mainweatherfetch() will run again every 30 seconds
 in the condition  the watch observe when somehting happen or changed in the main issued value 
 when mainisiued is turn to true and interval is not null(so the timer won't stack when i only check if mainisused) 
 it will run the mainweatherfetch again so data always refresh
...
/*
 ...
 the reason why i use <25 is because the total output the search can give is 25 
...
// it will be used below so the container wont be shown first and will only be show when a user choose from the search results
let weatheroutput = ref(false);

// array to use for iteration of vaues
const newday = ref([]);

// I use this code(one - seven)  to iterate the hour by hour array this one  for  next day  until the last day
const one = [
...
// this function will get the data then convert it to day
...
// check if the value is night or day using the val from the api
...
// set the variable to the current day then next and so on like the one two three from above
function button1() {
  newday.value = one;
  day = weather_daily.time[0];
}
...
onMounted(() => {
  georesultfinal.list.name = "Mabalacat";
  country.value = georesultfinal.list.country = "Philippines";
  georesultfinal.list.latitude = 15.22303;
  georesultfinal.list.longitude = 120.57117;
  mainweatherfetch();
});
</script>
...
<template>
  ...
  <div>
    <!--v-model openmeteogeo.name is changes depending on what is typed on the search bar-->
    <div class="search-container">
      <input
        class="search"
        type="search"
        v-model="openmeteogeo.name"
        placeholder="Search"
      />
      <!--this will iterate the array and mark the index so it can be used below-->
      <div
        class="output-container"
        v-for="(openmeteoarray, index) in georesult.list"
        :key="index"
      >
        <!--it will change the value of listvar.value so it can use it for the weather output for the mainweatherfetch()-->
        <button v-on:click="listvar = index">
          {{ openmeteoarray.name }}{{ ", " + openmeteoarray.country }}
        </button>
      </div>
    </div>
    <br /><br /><br />
  ...
  <div v-if="weatheroutput">
    ...
    <div class="realtime-container">
      ...
      <div class="current-time-weather">  
        <div class="icon-container">
          <!--wmo[][].image the two brackets are for the "string 1" [to check if its day or night]
          in here it will take the weather_current weather code and convert it to string
          checkdatornight is the function it will check if its day or night
          -->
        ...
        <!--this is where linkoutput is used -->
        <a class="button-view" v-bind:href="linkoutput" target="_blank">Location</a>
      </div>
    ...
    <div class="grid-secondary-container">
      ...
      <div class="hourbyhour">   
        <!--iterate an array by using the new value of newday thhat changes based on what hte button above click
         that uses function that is set above(one,two, - seven)  -->
          <div class="hourbyhour-container" v-for="iterate in newday">
              <div class="hourbyhouricon">
                ...
                <div class="values">
                    ...
                    <!--split the time output  2025-12-07T00:01 to 00:01 -->
